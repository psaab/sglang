...
def get_free_port():
    for family, socktype, proto, _, sockaddr in get_addrinfos_for_bind(0):
        try:
            with socket.socket(family, socktype, proto) as s:
                s.bind(sockaddr)
...
def launch_dummy_health_check_server(host, port, enable_metrics):
    ...
    def run_server():
        try:
            ...
            logger.info(f"Dummy health check server stopped at {NetworkAddress(host, port).to_host_port_str()}")
...
def get_local_ip_by_remote() -> Optional[str]:
    ...
    for dns_host, dns_port in [("2001:4860:4860::8888", 80), ("8.8.8.8", 80)]:
        try:
            infos = socket.getaddrinfo(
                dns_host, dns_port, socket.AF_UNSPEC, socket.SOCK_DGRAM, 0,
